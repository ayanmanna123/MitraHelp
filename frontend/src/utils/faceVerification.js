...
export const performFaceComparison = async (idImage, selfieImage) => {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new Error('Face verification timed out')), 10000);
  });
  ...
  try {
    ...
    console.warn('Face comparison failed or timed out, using simulation:', error);
    ...
    return await simulateFaceComparison();
  }
};
